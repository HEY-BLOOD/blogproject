...
if __name__ == '__main__':
    ...
    tag_list = [
        'django', 'Python', 'Pipenv', 'Docker', 'Nginx', 'Elasticsearch',
    ...
    for _ in range(100):
        tags = Tag.objects.order_by('?')  # 随机排序
        tag1 = tags.first()
        tag2 = tags.last()
        cate = Category.objects.order_by('?').first()
        created_time = fake.date_time_between(  # 返回 2 个指定日期间的随机日期。三个参数分别是起始日期，终止日期和时区。
            start_date='-1y',  # 1 年前
    ...
    for post in Post.objects.all()[:20]:
        ...
        delta_in_days = '-' + str(
            (timezone.now() - post_created_time).days) + 'd'
