...
def deploy(c):
    ...
    supervisor_program_name = 'django-blogproject'

    project_root_path = '~/apps/django-blogproject/'
    ...
    with c.cd(project_root_path):
        ...
        # c.run('source {}venv/bin/activate').format(project_root_path)
        c.run('pip3 install -r requirements.txt')
        c.run('python3 manage.py migrate')
        c.run('python3 collectstatic')
